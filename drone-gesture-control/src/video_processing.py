...
class VideoProcessing:
    def __init__(self, gstream_def:str, estimation_mode_path:str, classification_mode_path:str, topology_path:str, labels_path:str):
        ...
        self.buffer_size = 7
        self.poses_buffer = collections.deque(self.buffer_size*[str(None)], self.buffer_size)
        self.processing_times = collections.deque(self.buffer_size*[0.], self.buffer_size)
        
        video_fps = 10
        frameSize = (224, 224)
        date_str = datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S")
        self.video_recording = cv2.VideoWriter('live_processing_{}.avi'.format(date_str),cv2.VideoWriter_fourcc(*'DIVX'), video_fps, frameSize)
        ...
        while True:
            ...
            if re:
                ...
                if label_pose:
                    ...
                    image = cv2.putText(image, label_pose, (10,25), cv2.FONT_HERSHEY_DUPLEX, .7, (0,), 2, cv2.LINE_AA)
                    image = cv2.putText(image, label_pose, (10,25), cv2.FONT_HERSHEY_DUPLEX, .7, (255,255,255), 1, cv2.LINE_AA)
                fps = self.get_fps()
                if fps:
                    fps = 'FPS: {:.2f}'.format(fps)
                    image = cv2.putText(image, fps, (10,image.shape[0]-10), cv2.FONT_HERSHEY_DUPLEX, .7, (0,0,0), 2, cv2.LINE_AA)
                    image = cv2.putText(image, fps, (10,image.shape[0]-10), cv2.FONT_HERSHEY_DUPLEX, .7, (255,255,255), 1, cv2.LINE_AA)
    ...
    def get_pose(self) -> str:
        ...
        if self.poses_buffer.count(latest_pose) >= int(self.buffer_size*.6):
            return latest_pose
        else:
            return None
