    ...
    while True:
        ...
        if label and (drone.vehicle.mode.name == "GUIDED"):
            ...
            if label == "T":
                if not drone.vehicle.armed:  # Arm
                    ...
                    if (
                        drone.vehicle.location.global_relative_frame.alt < THRESHOLD_ALT
                    ):  # Disarm if landed
                ...
                if (
                    ...
                    while drone.vehicle.location.global_relative_frame.alt < (
                        takeoff_alt - THRESHOLD_ALT
            ...
            # Go front
                ...
                label == "Traffic_FrontStop" or label == "Stand_RightArmRaised"
            ) and drone.vehicle.armed:
                x, y = 2.0, 0.0  # meters
                yaw = drone.vehicle.attitude.yaw
                drone.send_global_velocity(
                    x * np.cos(yaw) - y * np.sin(yaw),
        ...
        print(
            "FPS: {fps:.2f}\tCurrent label: {lab1}\tSend order: {lab2}".format(
                fps=video_processing.get_fps(), lab1=latest_label, lab2=label
            ).ljust(80)[:80],
            end="\r",
        )
except:
